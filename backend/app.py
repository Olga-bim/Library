...
from extentions import db  # Импортируйте db из extensions.py
...
def about_website():
    log_function_time("about_website")
...
def about_website_creator():
    log_function_time("about_website_creator")
...
def add_customer():
    log_function_time("add_customer")
...
def list_books():
    ...
    return render_template('books.html', books=books,  categories=categories, authors=authors, years=years, delete_success=delete_success)
...
def add_loan():
    if request.method == 'POST':
        new_loan = Loan(
            ...
            loan_date=datetime.utcnow()  # Используем текущее время для даты займа
        )
...
def edit_loan(id):
    ...
    if request.method == 'POST':
        ...
        # Дата возврата обновляется автоматически при редактировании
        book = Book.query.get_or_404(loan.book_id)
        loan_type = LoanType(book.loan_type)
        loan.set_return_date(loan_type)  # Устанавливаем дату возврата заново
